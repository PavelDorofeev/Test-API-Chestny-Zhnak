                ...
                )
{
    ...
    //-----------------------------------------------------------------------------
    // ОХPЕНЕННО ВАЖНО ! СОЗДАВАТЬ НА СТЕКЕ , НО НЕ В КУЧЕ ОКНА
    // Т.К. ОНИ ТОЧНО УДАЛЯТСЯ ПРИ ВЫХОДЕ ОТСЮДА
    // С КУЧЕЙ ЕСТЬ НЮАНСЫ, КОГДА ПОСЛЕ ПЕPВОГО ОКНА СРАЗУ ВЫЗЫВАЕТСЯ ВТОPОЕ
    // А ПЕPВОЕ НЕ УСПЕВАЕТ ЕЩЕ ЗАКPЫТЬСЯ И ВИСИТ ФОНОМ
    //-----------------------------------------------------------------------------

    Smart_Process_Dlg dlg( parent ,
    ...
    // ------------------------------------------------------------------------------------------------------
    // странный дефект при статическом вызове  Transparency_Dlg_Sides::showWithTransparentWrapperDlg()
    // выход по Esc из окна, не освобождает девать (RELEASE_DEVICE не срабатывает)
    // ------------------------------------------------------------------------------------------------------

//    Transparency_Dlg_Sides wrapper ( parent ); // !!!! parent =0  это в отдельном окне будет

//    //qp::meta::cnct( &dlg , SIGNAL(closeWidget()) , &wrapper, SIGNAL(accepted()) );

//    qp::gui::setWindow_woTitle__woFrame_woToolsBtn( &wrapper );

//    wrapper.set_proportions( 1 , 3 , 1 , 1 , 10 , 1);

//    //dlg.setSizeGripEnabled( false );
//    wrapper.setSizeGripEnabled( false );

//    wrapper.set_centralWidget( &dlg );
...
//    wrapper.showMaximized();
    ...
    dlg.exec(); // именно отслеживаем события Smart_Process_Dlg, а не Transparency_Dlg_Sides !!

    //int ret = Transparency_Dlg_Sides::showWithTransparentWrapperDlg( dlg , parent );
    ...
    //dlg.setParent( parent ); // важно убрать владельца Transparency_Dlg_Sides

    // wrapper->deleteLater(); // dlg удалится автоматом как child of wrapper

    // dlg->deleteLater(); // надо отдельно удалять (почему-то...)
...
void setWindow_woToolsBtn(QWidget * wgt)
{
    ...
    //убираем с экрана кнопки закрыть, свернуть,развернуть
...
void setWindow_woTitle__woFrame_woToolsBtn(QWidget * wgt)
{
    ...
    //убираем с рамку с окна и заголовок сверху
    setWindow_woFrame_and_woTiltle(wgt);

    //убираем заголовок сверху
...
QMessageBox::StandardButton msg( QWidget *parent,
                                 const QString &title,
                                 const QString &text,
                                 ...
                                 QMessageBox::StandardButtons buttons)
{
    ...
    {
        int rr = msg.warning(parent , title , text, buttons );
        return (QMessageBox::StandardButton) rr ;
    }
