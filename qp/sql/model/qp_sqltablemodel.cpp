...
QVariant QpSqlTableModel::get_displayData( const QModelIndex &idx ) const
{
    ...
    if( unixtime_to_string.contains( col)) // == FT_UNIX_TIME_TO_STRING
    {
        return QDateTime::fromTime_t( QSqlTableModel::data( idx , Qt::DisplayRole ).toInt() ).toString(unixtime_to_string.value( col ) );
    }
    else if( int_to_text.contains( col ) )
    {
        int ival = QSqlTableModel::data( idx , Qt::DisplayRole ).toInt();

        if ( int_to_text.value( col ).contains( ival ) )
            return int_to_text.value( col ).value( ival );
        else
            return "";
    }
...
QString QpSqlTableModel::selectStatement() const
{
    ...
    for (int col = 0; col < baseColumnCount; ++col)
    {
        ...
        if(calc_columns.contains(fldName) )
        {
            ...
            QString relTableAlias = QString(prefixRelTbl+"_%1").arg(col);
        ...
        else if ( relations2.contains( fldName ) )
        {
            ...
            // -----------------------------------------------
            if( ! relation.externalDbName.isEmpty())
            {
                int extDbNumber = attachedDatabases.indexOf( relation.externalDbName );

                appendedDb = relation.externalDbName;//QString("%1_%2").arg(prefixApndDb).arg(extDbNumber);
            }
            //-------------------------------------

            QString relTableAlias;

//            if( ! appendedDb.isEmpty())
//            {
//                relTableAlias = QString("%1.%2").arg(appendedDb).arg(relation.extTblName);//.arg(col);
//            }
//            else
//            {
                relTableAlias = QString("%2_%3").arg(prefixRelTbl).arg(col);
//            }
...
//            QString arg1 = relation.idField1Name;
    ...
    foreach(QString colName,  relations2.keys())
    {
        ...
        QString relTableAlias = QString(prefixRelTbl+"_%1").arg(relCol);


        sExtLeftJoins.append(QLatin1String(" \nLEFT JOIN "));

        //QString extApndDbAlias;

        if( ! relation.externalDbName.isEmpty())
        {
            // для связи с таблицей из другой базы данных
            sExtLeftJoins.append( relation.externalDbName );
            sExtLeftJoins.append(QLatin1String(".")); // BB.equipments
            sExtLeftJoins.append(relation.extTblName);
            sExtLeftJoins.append(" as ");
            sExtLeftJoins.append(relTableAlias);
        }
        else
        {
            sExtLeftJoins.append(relation.extTblName);
            sExtLeftJoins.append(QLatin1String(" as "));  // contractors relTbl_18
            sExtLeftJoins.append(relTableAlias);
        }

        sExtLeftJoins.append(QLatin1String(" ON "));

        sExtLeftJoins.append(tableName());
        sExtLeftJoins.append(QLatin1String("."));
        sExtLeftJoins.append(relation.idField1Name);
        ...
        if( ! relation.externalDbName.isEmpty())
        {
            ...
            sExtLeftJoins.append(relTableAlias);
            sExtLeftJoins.append(QLatin1String("."));
            sExtLeftJoins.append(relation.idField2Name);
        }
        else
        {
            sExtLeftJoins.append(relTableAlias);
            sExtLeftJoins.append(QLatin1String(".")); // relTbl_18.id
            sExtLeftJoins.append(relation.idField2Name);
        }
    ...
    // добавляем подключение внешних баз данных
    // это если внешние связи полей ведут к другим базам данных
    // ATTACH DATABASE 'C:\ProgramData\BIT_DriverKKT\aaa.db' as BB;
    for( int idx=0; idx < attachedDatabases.count() ; idx++ )
    {
        // базы должны быть подключены один раз предварительно
        //query.prepend( QString("ATTACH DATABASE '%1' as %2_%3;\n").arg( attachedDatabases.at( idx ) ).arg(prefixApndDb).arg(idx));
    }
