...
const QLatin1String QpCmnSqlTableModel::prefixRelTbl = QLatin1String("relTbl");
const QLatin1String QpCmnSqlTableModel::prefixApndDb = QLatin1String("ApndDb");
...
bool QpCmnSqlTableModel::setIntToTextMap( const QString & fldName, const QHash<int,QString> &mm)
{
    qDebug() <<"3243242343"<<mm;

    int col = indexOf( fldName ) ;

    if( col == -1 )
        return false;

    int_to_text.insert( col , mm );
