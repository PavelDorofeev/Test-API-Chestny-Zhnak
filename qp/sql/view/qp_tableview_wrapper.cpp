...
#include <QPainter>
#include <QPaintEvent>
#include "qp/gui/delegates/combobox_delegate.h"
#include "qp/gui/delegates/comboboxdelegate2.h"
#include "vertical_header.h"
#include "horizontal_header.h"
#include "qp/gui/delegates/checkbox_delegate.h"
#include "qp/gui/delegates/defaultstyleditemdelegate.h"
#include "qp/gui/delegates/qprelationdelegate.h"
#include "qp/qp.h"
...
#include "qp/gui/delegates/date_delegate.h"
#include "horizontal_header.h"
...

{
    ...
    hor = new Horizontal_Header( this);
    ...
    //ver = new Vertical_Header( this);

    //setVerticalHeader( ver );
...
void QpTableViewWrapper::resizeColumnsToContents()
{
    hor->resizeSections( QHeaderView::ResizeToContents );
}

void QpTableViewWrapper::resizeRowsToContents()
{
    //ver->resizeSections( QHeaderView::ResizeToContents );
}
...
void QpTableViewWrapper::slot_editingEnabled_clicked(bool editOn) // from chk box toggled
{
    ...
    qpAbstractModel()->set_editable( editOn ); // !!!!!
                                            ...
                                            bool hideBtns ) // from external code
{
    ...
    qpAbstractModel()->set_editable( On ); // !!!!!
                                             ...
                                             bool hideBtns  )
{
    ...
    if(On)
    {
        ...
        if(act_CopyRow  == 0)
        {
            ...
            act_CopyRow = new QAction( trUtf8("copy row"), this);
        ...
        act_CopyRow->setIcon( QIcon(":/view/img/btn-db/copy.svg") );
        tlbx->ui->btn_copy->setEnabled( true );
        tlbx->ui->btn_copy->setVisible( true );
        tlbx->ui->btn_copy->setToolTip("копирование строки");
        //        }
        //        else
        //        {
        //            //act_CopyRow->setDisabled( true );
        //            act_CopyRow->setVisible( false );
        //            //act_CopyRow->setToolTip("Есть уникальные поля, копирование строк не допустимо.");
        //            //act_CopyRow->setIcon( QIcon(":/view/img/btn-db/copy-dis.svg") );

        //            tlbx->ui->btn_copy->setVisible( false );
        //            //tlbx->ui->btn_copy->setDisabled( true );
        //            //tlbx->ui->btn_copy->setToolTip("Есть уникальные поля, копирование строк не допустимо.");
...
QpTableViewWrapper::~QpTableViewWrapper()
{
    qDebug() << "~QpTableViewWrapper()"<<objectName();

    delegts.clear();

}

void QpTableViewWrapper::setModel( QpCmnSqlTableModel *newMdl)
{
    mdl = newMdl;
    ...
    if( ! qobject_cast<QpCmnSqlTableModel*>( QTableView::model() ))
    {
    ...
    hasUniqueFields = mdl->hasUniqueFields();

}

void QpTableViewWrapper::setTempModel( QpTempSqlTableModel *newMdl)
{

    QTableView::setModel( (QAbstractTableModel*)newMdl );

    //    qpMdl = (QpAbstractTableModel*)newMdl;
...
void QpTableViewWrapper::slot_setVisibleExRelIdColumns(bool visible)
{
    ...
    QpCmnSqlTableModel * mdl = Model();

    foreach(QString fldName, mdl->specialFld.keys())
    {
        //qDebug() << "specialFld setColumnHidden " << fldName << ! showRelExColumns;

        if( mdl->specialFld[ fldName ] !=  QpCmnSqlTableModel::CALC_FLD)
        {
            int Col = mdl->getBaseRec().indexOf( fldName );

            //            qDebug() << "setColumnHidden: " << fldName;

            setColumnHidden( Col , ! showRelExColumns);
        }
    }

    QHashIterator<QString , QpColumnInfo> it( mdl->colInfo);

    while( it.hasNext() )
    {
        it.next();

        //        qDebug() << "COLUMN_INFO: " << it.value().header << it.value().is_visible ;
    ...
    if( mdl->getPriColumn() >=0)
        setColumnHidden( mdl->getPriColumn() , ! showRelExColumns);

    if( mdl->subAccountingFilter.count()>0 )
    {
        foreach( QString col, Model()->subAccountingFilter.keys())
        {
            //qDebug() << "col "<< col <<  Model()->baseRec.indexOf( col );
            setColumnHidden( mdl->baseRec.indexOf( col ) , ! visible );
        }
    }
...
void QpTableViewWrapper::setContextMenuItemsVisibleAfterFieldSelected()
{
    ...
    else if( Model()->editable )       // форма редактируемая
    {
        ...
        if( act_CopyRow != 0 && ! hasUniqueFields ) // при наличии уникальных полей запрещае копирование
...
bool QpTableViewWrapper::vrt_clearField(const QModelIndex &idx)
{
    ...
    if ( qpAbstractModel()->isRelationalColumn( col ) )
    {
...
bool QpTableViewWrapper::vrt_viewRow(int row)
{
    ...
    emit sig_viewRow( row ); // you have to realize this yourself in derived class
...
bool QpTableViewWrapper::vrt_editRowDlg(int row)
{
    ...
    emit sig_editRow( row ); // you have to realize this yourself in derived class
...
bool QpTableViewWrapper::vrt_copyRow( int srcRow )
{
    ...
    qDebug() << "editStrategy" << mdl->editStrategy()<< " row id" << id << "srcRow" <<srcRow;

    if( mdl->submit() )

        /*
                Перед копированием надо сделать submit
                потому что, уже могут быть не сохраненные данные
                та же строка уже копированная, но не сохраненная
            */
    {
        qDebug() << "не сохраненные данные: были" ;
    }
    ...
    int newRow = mdl->rowCount();

    QSqlRecord srcRec = mdl->qp_record( srcRow );

    qDebug() << "copy row srcRec" << srcRec;

    bool bb = mdl->insertRow( newRow );

    if( ! bb )
    {
        qCritical() << "QpTableViewWrapper::copyRow  insertRow ERROR : " << mdl->lastError().text();
        return false;
    }

    bool bbb = mdl->removePrimaryKeys_fromRec( srcRec, QpSqlTableModel::INSERT );

    // ----------------------------------------------------

    //    QSqlRecord srcRec2 = mdl->record(srcRow);

    //    QSqlIndex priKey = mdl->primaryKey();
    ...
    //        priLst.append( fld.name());

    //    }

    //    for( int col=0; col < srcRec2.count(); col++ )
    //    {
    //        if( priLst.contains( srcRec2.fieldName( col )))
    //            continue;

    //        bool bb = mdl->setData( mdl->index( newRow , col ), srcRec.value( col ), Qt::EditRole );

    //        qDebug() << srcRec2.fieldName( col)<< "isGenerated"<< srcRec.isGenerated( col ) << srcRec.value( col ).isNull() << srcRec.value( col ).isValid();


    //    }

    QSqlRecord insrtRec = mdl->record( newRow );


    bbb = mdl->qp_setRecord( newRow , QpSqlRecord( mdl , srcRec ) );

    for( int col=0; col < insrtRec.count(); col++ )
    {
        qDebug() << insrtRec.fieldName( col)<< "isGenerated"<< insrtRec.isGenerated( col ) << insrtRec.value( col ).isNull() << insrtRec.value( col ).isValid();
    }
    ...
    bool sbmt = mdl->submit();

    if( ! sbmt )
    {
        /*
            БУДЬ ОСТОPОЖЕН С
            "UNIQUE constraint failed: cashiers.fio Unable to fetch row"
            КОПИPОВАНИЕ НЕ ПPОЙДЕТ
        */
        qDebug()<< "УНИКАЛЬНЫЕ ПОЛЯ ОДНАКО МОГУТ БЫТЬ"<< mdl->lastError().text();

        QMessageBox::warning(this ,
                             qp::dbg::warning ,
                             QString("submit: table '%1'\nerror:%2\nin row: %3")
                             .arg(mdl->tableName())
                             .arg(mdl->lastError().text())
                             .arg(newRow)
                             );
...
bool QpTableViewWrapper::slot_copyRowBtnClick()
{
    ...
    QHash< QString , QpColumnInfo>::ConstIterator it = mdl->colInfo.constBegin();

    while ( it != mdl->colInfo.constEnd() )
    {
        if( it.value().unique )
        {
            QMessageBox::warning(this ,
                                 qp::dbg::warning ,
                                 QString("copying is unavailable while exist unique fileds (in table : '%1' - '%2')").
                                 arg(mdl->tableName()).
                                 arg(it.key() )
                                 );
            return false;
...
bool QpTableViewWrapper::vrt_removeRow(int row)
{
    ...
    if(bb)
    {
        ...
        //        if( Model()->editStrategy() == QSqlTableModel::OnManualSubmit)
        //        {
        //            setRowHidden( row , true ); // при OnManualSubmit очищается, н оне удаляется
        //        }
...
{
    qDebug() << "QpTableViewWrapper::close()"<<objectName();

}


const QpSqlRecord QpTableViewWrapper::getBaseRec() const
{
    QpCmnSqlTableModel* pp = Model();

    if( pp !=0 )
        return pp->getBaseRec();
...
bool QpTableViewWrapper::vrt_doubleClicked(const QModelIndex & idx_dblClk , bool &needsSubmitAfter)
{
    ...
    QString fldName = getBaseRec().fieldName( col );
    ...
    if( qpAbstractModel()->isRelationalColumn( col ) ) // relations
    {

        QpSqlRelation relation = qpAbstractModel()->getRelationInfoForColumn( fldName );

        // ---------------------------------------------------------
        //  if is subaccounting presents but this toggled off
        // ---------------------------------------------------------

        if( qpAbstractModel()->rel_bindings.contains( fldName ) )
        {
            QpSubAccnt * subaccnt = qpAbstractModel()->rel_bindings[ fldName ];
            ...
            if( ! sub_on_fld_name.isEmpty() )
            {
                ...
                int sub_on = Model()->data( Model()->index( row , qpAbstractModel()->baseRec.indexOf( sub_on_fld_name ))).toInt( );
        ...
        int relCol = qpAbstractModel()->baseRec.indexOf(relation.idField1Name);
        ...
        if( qpAbstractModel()->rel_bindings.contains(fldName))
        {
            ...
            QpSubAccnt *subAccnt = qpAbstractModel()->rel_bindings.value( fldName );


            int parentCol =  qpAbstractModel()->baseRec.indexOf( subAccnt->fld1 );

            int relPrntIdCol = qpAbstractModel()->getRelIdColumn( parentCol );
        ...
        if( Model()->editStrategy() == QSqlTableModel::OnRowChange
                || Model()->editStrategy() == QSqlTableModel::OnManualSubmit ) // <=!!!
        {
            // ------------------------------------------------
            ...
            qDebug() << "set txt " << txt.toString()<< "to" << idx_dblClk;
...
bool QpTableViewWrapper::prepare_view( QpCmnSqlTableModel * mdl )
{
...
{
    QpCmnSqlTableModel* qpMdl = Model();

    if( ! qpMdl->getBaseRec().field( col ).isValid() )
    {
        //        QMessageBox::warning(this ,
        //                             qp::dbg::error_ ,
    ...
    QString fldName = qpMdl->getBaseRec().fieldName( col );
...
{
    QpSqlRecord bsRec = Model()->getBaseRec();

    if( ! bsRec.field( fldName ).isValid() )
    {
        QMessageBox::warning(this ,
                             qp::dbg::error_ ,
    ...
    setItemDelegateForColumn( bsRec.indexOf( fldName ) , cmbDeleg);

}

void QpTableViewWrapper::setDoubleWithPrecision( const QString & fldName , int precision)
{
    QpSqlRecord bsRec = qpAbstractModel()->getBaseRec();
    ...
    //    delegts.insert( fldName , new DoubleDelegate( precision, dd , 0 , this ) );
...
void QpTableViewWrapper::setRelationDelegate(const QString & fldName)
{
...
{
    if ( ! qpAbstractModel()->getBaseRec().field( fldName ).isValid() )
    {
        QMessageBox::warning(this ,
    ...
    int col = qpAbstractModel()->getBaseRec().indexOf( fldName );


    setItemDelegateForColumn( col , cmbDeleg);

    qpAbstractModel()->defaultVls.insert( col , -1);

}

void QpTableViewWrapper::setComboBoxDelegate2(const QString & fldName, const QVariantList &lst)
{
    if ( ! Model()->baseRec.field( fldName ).isValid() )
    {
        QMessageBox::warning(this ,
...
void QpTableViewWrapper::setDateTimeDelegate(const QString & fldName, int fullDateTime)
{
    if( ! Model()->baseRec.field( fldName ).isValid() )
    {
        QMessageBox::warning(this ,
    ...
    delegts.insert( fldName ,
                    new date_Delegate( fullDateTime , this)
                    );
...
QpCmnSqlTableModel* QpTableViewWrapper::Model() const
{

    return qobject_cast<QpCmnSqlTableModel*>( QTableView::model() );

}

QpCmnSqlTableModel* QpTableViewWrapper::qpAbstractModel() const
{
    /*
        Вот это финт! Надо проверить корретностьт работы
        Внимание работает dynamic_cast как ни странно ,
        выдает указатель на второй класс родителя в наследовании
    */

    QpCmnSqlTableModel * qpMdl = dynamic_cast<QpCmnSqlTableModel*>( QTableView::model() ); // Внимание работает dynamic_cast

    if( qpMdl ==0 )
    {
        qDebug() <<"463783973867386854";
        Q_ASSERT(1==5645654);
        return 0;
    }

    //    qDebug() << "dasdasd getBaseRec"<<qpMdl->getBaseRec();
...
QpCmnSqlTableModel* QpTableViewWrapper::model() const
{
    QpCmnSqlTableModel * mdl = qobject_cast<QpCmnSqlTableModel*>(QTableView::model());
...
void QpTableViewWrapper::commitData( QWidget *editor )
{
    /*
        Важно commitData не срабатывает
        для наших спец. полей с внешними связями
        где значение выбирается через открытие диалога
        выбора строки из внешней таблицы

        и для QpTempSqlTableModel значение вторым этапом будет пустое
    */

    qDebug()<<"QpTableViewWrapper::commitData" << currentIndex() << model()->tableName();

    //    int col = currentIndex().column();

    //    QString fldName = getBaseRec().fieldName( col );

    //    int relCol = getBaseRec().getRelIdCol( fldName );

    //    if( relCol != qp::db::COL_UNDEFINED )
    //    {
    //        QString relFldName = getBaseRec().fieldName( relCol );

    //        if ( getBaseRec().specialFld.value( relFldName ) == QpSqlRecord::RELATION_ID_EXT_FLD )
    //        {
    //            return;
    //        }
    //    }
...
//void QpTableViewWrapper::mouseDoubleClickEvent(QMouseEvent *evt )
//{
//    if( ! currentIndex().isValid())
//    {
//        qDebug() << "QpTableViewWrapper::mouseDoubleClickEvent" << currentIndex();
//        return;
//    }
...
void QpTableViewWrapper::currentChanged(const QModelIndex &current, const QModelIndex &previous)
{
    ...
    {
        // -----------------------------------------------------------------------------------
        // this needs to call submit method when current changed (dirty) row lost the focus
        // -----------------------------------------------------------------------------------
        ...
        if( previous.row() != -1 && previous.row() != current.row())
        {
            ...
            for(int col=0; col < Model()->baseRec.count(); col++)
            {
                ...
                if( Model()->specialFld.contains( fldName )
                        && ( Model()->specialFld[ fldName ] == QpCmnSqlTableModel::CALC_FLD
                             || Model()->specialFld[ fldName ] == QpCmnSqlTableModel::SUBACCOUNT_ENABLE_FLD ))
                    continue;
            ...
            if( to_submit )
            {
                ...
                if( ! Model()->submit() )
                {
                    QMessageBox::warning( this,
                                          "error",
                                          "submit() returns false [45635343565]");
                }
            }
        }
    }
