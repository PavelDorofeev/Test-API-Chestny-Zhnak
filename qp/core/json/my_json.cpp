...
QByteArray serializeHash(const QVariantHash &hh, bool *success, JSON_OPTIONS options,int level)
{
    QByteArray tabs;
    QByteArray tabsFields;
    ...
    QVariantHash::ConstIterator it = hh.constBegin();

    while(  it != hh.constEnd() )
    {
        //bool otherSuccess = true;
        QByteArray serializedValue = serialize1( it.value(), success, options, level);
...
QByteArray serialize1(const QVariant &data, bool *success, JSON_OPTIONS options,int level /*= 0*/)
{
    ...
    { // variant is a hash?
        ba = serializeHash( data.toHash(), success, options, level );
    }
